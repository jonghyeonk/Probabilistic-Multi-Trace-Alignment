...
for p in p_list:
    ...
    if name == 'road_weibull4':
        ...
        elif name in ['credit-card_new', 'mccloud_new']:
            df_train = data[['Case','Timestamp', 'Activity',  'Resource', 'System']]
    ...
    test = df_test.groupby('Case').apply(lambda x: apply(x, actlist, A1list, A2list, power = p)).reset_index(drop=True)
    ...
    encoded_df.to_csv(dir_home+"/data_trans/" + name +"_aggregate_" + str(p) + ".csv", index= True)
    ...
    encoded_df = pipe.fit_transform(df)
    encoded_df.to_csv(dir_home+"/data_trans/" + name +"_bool_" + str(p) + ".csv", index= True)
    ...
    encoded_df = pipe.fit_transform(df)
    encoded_df.to_csv(dir_home+"/data_trans/" + name +"_index_" + str(p) + ".csv", index= True)
    ...
    encoded_df.to_csv(dir_home+"/data_trans/" + name +"_aggngram_" + str(p) + ".csv", index= True)
    ...
    encoded_df = pipe.fit_transform(df)
    encoded_df.to_csv(dir_home+"/data_trans/" + name +"_laststate_" + str(p) + ".csv", index= True)
